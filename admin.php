...
<?php 
  $modal_set2 = find_all_applications();
?>
...
if(is_post_request()) {
  $job = [];
  $today = date("Ymd");
  $job['name'] = isset($_POST['name']) ? $_POST['name'] : '';
  $job['class'] = isset($_POST['class']) ? $_POST['class'] : '';
  $job['type'] = isset($_POST['type']) ? $_POST['type'] : '';
  $job['description'] = isset($_POST['description']) ? $_POST['description'] : '';
  $job['location'] = isset($_POST['location']) ? $_POST['location'] : '';
  $job['date'] = isset($today) ? $today : '';
...
<body>
  ...
  <div class="sb-site-container">
    ...
    <div class="ms-hero-page-override ms-hero-img-city ms-bg-fixed ms-hero-bg-dark-light">
      <div class="container">
        <div class="text-center">
          <h3>Welcome <?php echo isset($_POST['first_name']) ? $_POST['first_name'] : ''; ?></h3>
    ...
    <?php } ?>
      ...
      <!-- Second mymodal -->
    <?php while($m_modal2 = mysqli_fetch_assoc($modal_set2)) { ?>
    <div class="modal modal-warning" id="myaModal<?php echo $m_modal2['id']; ?>" tabindex="-1" role="dialog" aria-labelledby="myaModalLabel<?php echo $m_modal2['id']; ?>">
      <div class="modal-dialog animated zoomIn animated-3x" role="document">
        <div class="modal-content">
          <div class="modal-header">
            <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true"><i class="zmdi zmdi-close"></i></span></button>
            <h3 class="modal-title" id="myaModalLabel<?php echo $m_modal2['id']; ?>">Are you sure you want to delete <strong><?php echo $m_modal2['name']; ?></strong>?</h3>
          </div>
          <div class="modal-body">
            <h3>Deleting this application is permanent.</h3>
            <p class="lead">
              <?php echo $m_modal2['comment']; ?>
            </p>
          </div>
          <div class="modal-footer">

            <form action="<?php echo 'includes/app_delete.php?id=' . $m_modal2['id']; ?>" method="post">
              <fieldset>
                <div class="form-group">
                  <div class="col-md-10 col-md-offset-2">
                    <button type="submit" class="btn btn-raised btn-primary">Submit</button>
